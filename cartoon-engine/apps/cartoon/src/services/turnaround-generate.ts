...
export async function generateTurnaround(
  ...
  const asset = await createAssetWithProvenance(deps.db, {
    ...
    // Ракурс берётся из уже проверенной спецификации, а не разбирается обратно
    // из имени файла: имя — производная от ракурса, и выводить причину из
    // следствия значило бы сделать соглашение об именовании частью контракта.
    cameraAngle: angle.angle,
    characterId: character.id,
    // Версия — история ОДНОГО пути, и у каждого ракурса он свой. ¾ справа не
    // является версией ¾ слева: это разные ассеты, а не разные редакции одного.
    // Второй дубль того же ракурса ложится по своему пути (в имени seed) и тоже
    // получает версию 1 — какой из дублей принят, модель пока не выражает, и
    // подменять это решение автоинкрементом нельзя.
    version: 1,
    supersedesId: null,
    provenance: {
      producedBy: 'provider',
