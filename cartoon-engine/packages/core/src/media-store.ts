...
export class MediaStore {
  ...
  /**
   * Заменить байты по УЖЕ СУЩЕСТВУЮЩЕМУ пути.
   *
   * Единственное исключение из правила «поверх не пишем», и оно узкое намеренно.
   *
   * Почему исключение понадобилось. Версия в этой модели — это тот же путь и
   * другие байты: `Asset.supersedesId` описан как «предыдущая версия ПО ТОМУ ЖЕ
   * ПУТИ», `listCurrentAssets` группирует именно по пути, а уникальна пара
   * путь+версия. До сих пор версии появлялись только от руки: человек подменял
   * файл, а `assets import` описывал то, что уже лежало на диске. Своей версии
   * движок сделать не мог — `receive` отказывает, — и эталон, оказавшийся
   * неверным, нечем было заменить, не солгав реестру: положив новый файл рядом,
   * мы получили бы у персонажа два «текущих» эталона сразу, и запрос вернул бы
   * тот, чей путь меньше по алфавиту, то есть устаревший.
   *
   * Чем это НЕ является. Не откатом защиты `receive`: тот по-прежнему не пишет
   * поверх ничего и никогда. Здесь другое действие с другим именем, и звать его
   * осмысленно ровно в одном случае — когда прежние байты уже описаны отдельной
   * строкой Asset и остаются в реестре как история, на которую можно сослаться.
   *
   * Отсутствующий путь — ошибка, а не повод создать файл. Создание — это
   * `receive`, и делать оба действия одним методом значило бы вернуть ту самую
   * неразличимость, ради которой заведён отдельный метод.
   *
   * Замена атомарна: байты ложатся во временный файл рядом и переезжают на
   * место одним `rename`. Оборванная запись не оставит половину эталона — на
   * месте будут либо прежние байты целиком, либо новые целиком.
   */
  async replaceExisting(relativePath: string, bytes: Buffer): Promise<MediaFacts> {
    const absolutePath = this.resolve(relativePath);

    if (!(await this.exists(relativePath))) {
      throw new MediaWriteError(
        `Файл ${toPosix(relativePath)} не существует. Замена работает только по ` +
          'известному пути: создание нового файла — это receive().'
      );
    }

    // Временный сосед, а не общий tmp: rename атомарен лишь в пределах одной
    // файловой системы, а сосед по каталогу заведомо на ней же.
    const temporaryPath = `${absolutePath}.replacing-${process.pid}`;

    try {
      await writeFile(temporaryPath, bytes, { flag: 'wx' });
      await rename(temporaryPath, absolutePath);
    } catch (error) {
      // Прибираем за собой: иначе следующий вызов упрётся в чужой хвост и
      // сообщит о занятом пути вместо настоящей причины.
      await unlink(temporaryPath).catch(() => undefined);
      throw error;
    }

    // Как и в receive: описываем то, что реально легло на диск.
    return this.describe(relativePath);
  }
