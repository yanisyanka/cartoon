...
async function main(): Promise<void> {
  ...
  try {
    ...
    // Считаются ТЕКУЩИЕ, а не все строки. Задний эталон стал версионируемым:
    // суждение о «настоящих 180°» человек может пересмотреть, и тогда прежняя
    // строка остаётся в реестре как история. Две строки одного персонажа — это
    // версия, а не дубль, и различает их supersedesId: перекрытая версия та, на
    // которую кто-то ссылается. Считать сырые строки значило бы объявлять
    // нарушением саму историю.
    check(
      'у персонажа не больше одного текущего заднего эталона',
      (() => {
        const superseded = new Set(
          assets.map((a) => a.supersedesId).filter((id): id is string => id !== null)
        );
        const byCharacter = assets
          .filter((a) => a.role === 'ref-back' && !superseded.has(a.id))
          .map((a) => a.characterId);
        return new Set(byCharacter).size === byCharacter.length;
      })(),
      'два непересечённых задних эталона у одного персонажа — версия и дубль перепутаны'
    );
