 ...
 * только пустое и отказывается переписывать уже проставленное. Повторный запуск
 * поэтому физически не может изменить правильную запись.
 *
...
async function main(): Promise<void> {
  ...
  try {
    const turnarounds = (await listAssets(db)).filter(
      (asset) => asset.role === TURNAROUND_ROLE
    );

    if (turnarounds.length === 0) {
      console.log('Ракурсов в реестре нет — проставлять нечего.');
      return;
    }
    ...
    console.log(
      `\nПроставлено: ${filled} · уже верных: ${alreadyRight} · ` +
        `отказов: ${refused} · всего ракурсов: ${turnarounds.length}`
    );
